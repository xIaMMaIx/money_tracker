...
# ///////////////////////////////////////////////////////////////
main_container = None 
DB_FILENAME = "modern_money.db" # กำหนดชื่อ Database ตายตัวสำหรับ Web
...
def main(page: ft.Page):
    ...
    # [WEB FIXED] ปุ่มสำหรับเปิด Popup (เปลี่ยนจาก mic เป็น add/remove)
    # Action (on_click) จะถูกกำหนดใน Section 6 หลังจากประกาศฟังก์ชัน
    btn_expense = ft.FloatingActionButton(
        text=T("expense"), 
        icon="remove", 
    ...
    btn_income = ft.FloatingActionButton(
        text=T("income"), 
        icon="add", 
    ...
    def refresh_ui(new_id=None):
        ...
        # Calculate Net Worth (Balance - Debt)
        ...
        for r in rows:
            ...
            card = TransactionCard(r, call_delete, call_edit, d_font_delta, d_font_weight, is_new=(r[0] == new_id), minimal=False)
    ...
    def open_settings(e):
        def dummy_init(path):
            refresh_ui()
        open_settings_dialog(page, current_db, config, refresh_ui, dummy_init, cloud_mgr)

    btn_settings.on_click = open_settings

    # [NEW FUNCTION] Manual Add Dialog
    ...
    # ///////////////////////////////////////////////////////////////
    def build_full_view():
        # [แก้ไข 1] ลบ width=320 ออก เพื่อให้ Sidebar ยืดเต็ม Grid (Col 3)
        ...
        # [แก้ไข 2] ปรับสัดส่วน Grid เป็น 9 : 3 (Main ใหญ่ขึ้น Sidebar เล็กลง)
        return ft.ResponsiveRow([
            ft.Column([main_pane], col={"md": 9, "sm": 12}), # เดิม 8
            ft.Column([sidebar], col={"md": 3, "sm": 12})    # เดิม 4
        ])

    def init_web():
        nonlocal current_db
        if not os.path.exists(DB_FILENAME):
            print(f"Creating new DB: {DB_FILENAME}")
            
        current_db = DatabaseManager(DB_FILENAME)
        current_db.connect()
